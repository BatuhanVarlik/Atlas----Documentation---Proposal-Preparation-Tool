...
import { updateMilkReceptionModuleSchema } from '@/lib/validations/milkReceptionModule';
...
export async function PUT(request: Request, { params }: Params) {
  try {
    ...
    const parsed = updateMilkReceptionModuleSchema.safeParse(body);
    if (!parsed.success)
      return apiError('Geçersiz veri', 400, parsed.error.flatten());

    const existing = await prisma.milkReceptionModule.findUnique({ where: { id } });
    if (!existing) return apiError('Modül bulunamadı', 404);
    if (user.role === 'MEMBER' && existing.creatorId !== user.id)
      return apiError('Forbidden', 403);

    const mod = await prisma.milkReceptionModule.update({
      where: { id },
      data: parsed.data,
...
export async function DELETE(_req: Request, { params }: Params) {
  try {
    ...
    if (user.role === 'MEMBER' && existing.creatorId !== user.id)
      return apiError('Forbidden', 403);
    await prisma.milkReceptionModule.delete({ where: { id } });
