...
// Üretilen CIP .docx dosyasını diskten okuyup indirir.
export async function GET(_req: Request, { params }: Params) {
  try {
    const user = await requireAuth();
    const { id, docId } = await params;

    const doc = await prisma.cipGeneratedDocument.findUnique({
      where: { id: docId },
...
// Üretilen CIP belgesini siler: DB kaydı + diskteki dosya.
export async function DELETE(_req: Request, { params }: Params) {
  try {
    ...
    await prisma.cipGeneratedDocument.delete({ where: { id: docId } });
    try {
