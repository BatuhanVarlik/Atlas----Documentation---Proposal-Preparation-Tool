...
export async function POST(req: Request, { params }: Params) {
  try {
    ...
    const prevSnapshot = (last?.snapshot ?? null) as ModuleSnapshot | null;
    const changes = detectChanges(prevSnapshot, current);
    const nextNumber = (last?.revisionNumber ?? 0) + 1;
    const label = `RV-${String(nextNumber).padStart(2, '0')}`;

    const revision = await prisma.moduleRevision.create({
      data: {
        moduleId,
    ...
    // Son 10'dan fazlasını sil (en eskileri)
    const total = await prisma.moduleRevision.count({ where: { moduleId } });
    if (total > MAX_REVISIONS) {
      const stale = await prisma.moduleRevision.findMany({
        where: { moduleId },
